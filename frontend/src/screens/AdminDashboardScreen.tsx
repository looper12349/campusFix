...
const AdminDashboardScreen: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(fetchIssues({ isAdmin: true }));
  }, [dispatch, filters]);

  const handleRefresh = async () => {
    setRefreshing(true);
    await dispatch(fetchIssues({ isAdmin: true }));
    setRefreshing(false);
  };

  const handleIssuePress = (issueId: string) => {
    navigation.navigate('AdminIssueDetail', { issueId });
  };
  ...
  const renderContent = () => {
    ...
    if (items.length === 0) {
      return (
        <EmptyState
          title={hasActiveFilters ? 'No matching issues' : 'No issues reported'}
          message={
            hasActiveFilters
              ? 'Try adjusting your filters'
              : 'No students have reported any issues yet'
          }
        />
      );
    }
    ...
    return (
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const studentName = typeof item.createdBy === 'object' ? item.createdBy.name : undefined;
          const studentEmail = typeof item.createdBy === 'object' ? item.createdBy.email : undefined;
